import type { IndustryPack } from '@prisma/client';

import { apiSuccess, jsonResponse } from '@/lib/api-response';
import { handleApiError } from '@/lib/api-auth';
import {
  createPackWorkItem,
  listPackWorkItems,
} from '@/server/packs/work-items/work-item.service';
import { requireApiPack } from '@/server/packs/require-api-pack';

export function createWave4PackRouteHandlers(pack: IndustryPack) {
  return {
    async GET(request: Request) {
      try {
        const auth = await requireApiPack(pack);
        if (auth instanceof Response) return auth;

        const { searchParams } = new URL(request.url);
        const status = searchParams.get('status') ?? undefined;
        const page = Number(searchParams.get('page') ?? 1);

        const data = await listPackWorkItems(auth.session.organizationId, pack, {
          status,
          page,
        });
        return jsonResponse(apiSuccess(data));
      } catch (error) {
        return handleApiError(error, `${pack}.GET`);
      }
    },

    async POST(request: Request) {
      try {
        const auth = await requireApiPack(pack);
        if (auth instanceof Response) return auth;

        const body = await request.json();
        if (!body.customerId || !body.title) {
          return jsonResponse(
            {
              success: false,
              error: { code: 'VALIDATION', message: 'customerId و title الزامی است' },
            },
            400,
          );
        }

        const item = await createPackWorkItem(auth.session.organizationId, pack, {
          customerId: body.customerId,
          title: body.title,
          status: body.status,
          scheduledAt: body.scheduledAt ? new Date(body.scheduledAt) : undefined,
          dueAt: body.dueAt ? new Date(body.dueAt) : undefined,
          amount: body.amount,
          location: body.location,
          notes: body.notes,
          meta: body.meta,
        });

        return jsonResponse(apiSuccess(item), 201);
      } catch (error) {
        return handleApiError(error, `${pack}.POST`);
      }
    },
  };
}
