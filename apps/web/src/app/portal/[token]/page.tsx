    ...
    >
      <div className="mx-auto max-w-2xl space-y-4">
          ...
          <CardHeader>
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <CardDescription>
                  وضعیت حساب و پیگیری‌ها — اعتبار لینک تا{' '}
          ...
          {contactBits.length > 0 ? (
            <CardContent>
              <ul className="space-y-1 text-sm text-muted-foreground" role="list">
                {contactBits.map((bit) => (
                  <li key={String(bit)} dir={bit === customer.phone || bit === customer.email ? 'ltr' : 'rtl'}>
                    {bit}
                  </li>
                ))}
              </ul>
        ...
        <p className="text-center text-xs text-muted-foreground">
          این لینک محرمانه است؛ آن را فقط با افراد مجاز به اشتراک بگذارید. پس از انقضا دیگر قابل استفاده
          نیست.
        </p>
      </div>
    </div>
