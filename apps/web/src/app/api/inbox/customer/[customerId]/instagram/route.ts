...
import { getCustomerInstagramThread } from '@/server/messaging/inbox.service';
...
export async function GET(
  ...
  try {
    ...
    const data = await getCustomerInstagramThread(session.organizationId, customerId);
    if (!data) {
    ...
    return handleApiError(error, 'inbox.customer.instagram.GET');
  }
}
